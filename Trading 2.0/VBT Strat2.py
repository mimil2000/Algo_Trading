...
def custom_long_indicator(close, ma_window1, ma_window2, mstd_window, mean_window, min_entry_delay=12):
    ...
    meandiff = pd.DataFrame(mean).diff().values


    trend = np.where((meandiff > 0)
                     & (ma1diff > ma2diff)
                     & (mean > 0)
                     & (ma1diff > 0)
                     & (ma2diff > 0)
...
indic_long = vbt.IndicatorFactory(
    ...
    output_names=["value"]
).from_apply_func(custom_long_indicator, ma1_window=ma1_window, ma2_window=ma2_window, mstd_window=mstd_window,
...
indic_short = vbt.IndicatorFactory(
    ...
    param_names=["ma1_window", "ma2_window", 'mstd_window', 'mean_window'],
    output_names=["value"]
).from_apply_func(custom_short_indicator, ma1_window=ma1_window, ma2_window=ma2_window, mstd_window=mstd_window,
                  mean_window=mean_window)
...
meandiff = pd.DataFrame(mean.to_numpy(), index=btc_price.index).diff()
meanSTD = mean[mean_window] + 1.0 * mstd[mstd_window]
meanstd = mean[mean_window] - 1.0 * mstd[mstd_window]

fig = pf.plot(
    subplots=['trades', ('price', dict(title='Metrics', yaxis_kwargs=dict(title='Price'))), 'trade_pnl', 'cum_returns',
              'drawdowns'])
...
scatter = vbt.plotting.Scatter(
    data=meandiff*10,
    x_labels=meandiff.index,
    trace_names=[f"Meandiff{mean_window}"],
    add_trace_kwargs=dict(row=2, col=1),
    fig=fig
)
